...
def test_config_file():
    ...
    config = audb2.core.config.load_configuration_file(config_file)
    assert config == {}
    ...
    global_config = audb2.core.config.load_configuration_file(
        audb2.core.config.global_config_file
    )
    global_config.update(config)
    assert global_config['cache_root'] == '~/user'
    assert global_config['shared_cache_root'] == audb2.config.SHARED_CACHE_ROOT
    ...
    error_msg = (
        ...
        f"in the configuration file '{audb2.core.define.USER_CONFIG_FILE}'."
    )
    with pytest.raises(ValueError, match=error_msg):
        audb2.core.config.load_configuration_file(config_file)
