...
class Dependencies:
    ...
    def __contains__(self, file: str) -> bool:
        ...
        return (
            len(
                self._dataset.to_table(columns=["file"], filter=f"file == '{file}'"),
            )
            > 0
        )
    ...
    def archives(self) -> typing.List[str]:
        ...
        """
        return sorted(
            self._dataset.to_table(columns=["archive"])
            .to_pandas()
            .archive.unique()
            .tolist()
        )
    ...
    def attachments(self) -> typing.List[str]:
        ...
        """
        return (
            self._dataset.to_table(
                columns=["file"],
                filter=f"type == {define.DependType.ATTACHMENT}",
            )
            .to_pandas()
            .file.tolist()
        )
    ...
    def files(self) -> typing.List[str]:
        ...
        """
        return self._dataset.to_table(columns=["file"]).to_pandas().file.tolist()
    ...
    def media(self) -> typing.List[str]:
        ...
        return (
            self._dataset.to_table(
                columns=["file"],
                filter=f"type == {define.DependType.MEDIA}",
            )
            .to_pandas()
            .file.tolist()
        )
    ...
    def removed_media(self) -> typing.List[str]:
        ...
        return (
            self._dataset.to_table(
                columns=["file"],
                filter=f"type == {define.DependType.MEDIA} AND removed == 1",
            )
            .to_pandas()
            .file.tolist()
        )
    ...
    def tables(self) -> typing.List[str]:
        ...
        return (
            self._dataset.to_table(
                columns=["file"],
                filter=f"type == {define.DependType.META}",
            )
            .to_pandas()
            .file.tolist()
        )
    ...
    def load(self, path: str):
        ...
        if extension not in ["csv", "pkl", "parquet", "lance"]:
            raise ValueError(
                f"File extension of 'path' has to be 'csv', 'lance', 'pkl', or 'parquet' "
                f"not '{extension}'"
            )
        ...
        elif extension == "lance":
            self._dataset = lance.dataset(path)
